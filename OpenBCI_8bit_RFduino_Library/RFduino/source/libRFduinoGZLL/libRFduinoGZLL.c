...
int RFduinoGZLL_begin(device_t device)
{
  ...
  if (RFduinoGZLL_channel)
  {
    uint8_t channel_table[] = { RFduinoGZLL_channel - 1, RFduinoGZLL_channel + 24, RFduinoGZLL_channel + 49 };
    if (! nrf_gzll_set_channel_table(channel_table, countof(channel_table)))
      return 5;
  }
  else
  {
    uint8_t channel_table[] = NRF_GZLL_DEFAULT_CHANNEL_TABLE;
    if (! nrf_gzll_set_channel_table(channel_table, NRF_GZLL_DEFAULT_CHANNEL_TABLE_SIZE))
      return 5;
  }
  ...
  if (! nrf_gzll_set_device_channel_selection_policy(NRF_GZLL_DEVICE_CHANNEL_SELECTION_POLICY_USE_CURRENT))
	...
	nrf_gzll_set_timeslot_period(700);
